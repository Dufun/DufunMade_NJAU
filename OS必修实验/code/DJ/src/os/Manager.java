...
public class Manager {
    ...
    public Manager() {
        // 将控制台内容重定向到指定文件
        File ProcessResults = new File("./output2/ProcessResults-DJ.txt");
        try {
            //如果路径不存在则新建路径
        ...
        this.processschedulingthread = new ProcessScheduling(this);
    }
    ...
    public void start() {
        ...
        this.dashboard.StartButton(this.processschedulingthread.getJobManage().getAlljobnum()>=3);
        if (this.processschedulingthread.getJobManage().getAlljobnum()<3){
            this.processschedulingthread.getManager().getDashboard().label1.setText("并发作业数不足3,请点击“添加作业”按钮加至3后按下启动按钮开始!");
        }
        // 时钟线程启动 等待启动按钮按下开始计时
        this.clock.start();
        // 调度线程启动(至少等时钟开始后才会开始即按下启动按钮后)
        this.processschedulingthread.start();
    ...
    public void renameFile(int totalSeconds) {
        ...
        File oldName = new File("./output2/ProcessResults-DJ.txt");
        File newName = new File("./output2/ProcessResults-" + totalSeconds + "-DJ.txt");
