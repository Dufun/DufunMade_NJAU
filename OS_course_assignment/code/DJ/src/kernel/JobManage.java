...
public class JobManage {
    ...
    public JobManage(ProcessScheduling schedue) {
        ...
        this.alljobnum=0;
        // 把现有作业读入数组里
        File jobsInputFile = new File("./input2/jobs-input.txt");
        try {
    ...
    public Instruction[] readInstructionSet(int jobsId, int instructionNum) {
        File instructionFile = new File("./input2/" + jobsId + ".txt");
        try {
            // 读取对应指令集文件
    ...
    public void addInstructionSet(int jobId, int instructionNum) {
        File instructions = new File("./input2/" + jobId + ".txt");
    ...
    public synchronized void tryAddProcess() {
        for (int i = 0; i<this.processschedulingthread.getReserveQueue().size(); i++ ) {
            ...
            this.processschedulingthread.getManager().getDashboard().process( this.processschedulingthread.getManager().getClock().getCurrentTime()+"：["+"进入就绪队列1："
